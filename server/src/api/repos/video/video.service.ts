...
import { NonNullObject } from 'src/utils/types';
import { Video, VideoDocument } from '@repos/video/video.schema';
...
export class VideoService {
    ...
    async createVideo(video: NonNullObject): Promise<Video> {
        const newVideo = new this.videoModel(video);
        return newVideo.save();
    }

    async readVideo(id): Promise<object> {
        if (id.id) {
    ...
    async streamVideo(id: string, response: Response, request: Request) {
        try {
            ...
            if (range) {
                ...
                const vidoeStream = createReadStream(join(process.cwd(), `./public/${video}`), {
                    start,
                    end,
                });
    ...
    async update(id, video: VideoDto): Promise<Video> {
